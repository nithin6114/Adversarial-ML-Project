...
import os
import numpy as np
import imageio  # ✅ use imageio instead of scipy.misc
...
def log_output(sess, eval_logits, eval_preds, x, adv, initial_img, \
    ...
    # ✅ Save images using imageio, scaling to [0, 255] if necessary
    imageio.imwrite(os.path.join(eval_dir, 'original.png'), (initial_img[0] * 255).astype(np.uint8))
    np.save(os.path.join(eval_dir, 'original.npy'), initial_img)

    imageio.imwrite(os.path.join(eval_dir, 'adversarial.png'), (adv[0] * 255).astype(np.uint8))
